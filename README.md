...
> 整理日期：2026-07-24 | 持续更新

---

## 🚀 快速开始（三种用法，按需选择）

> **首次使用？** 双击运行 **`setup.cmd`** 配置本地路径（Python、PotPlayer），生成 `config.json`。只需配置一次。
...
6. **开源协议** — 本项目采用 [MIT License](LICENSE)
...
### 报告线路失效
线路失效？[点此提交 Issue](https://github.com/0xing1/vip-video-resources/issues/new?template=broken-line.md)，注明失效线路和测试链接。

### 添加新线路
1. Fork 本仓库
2. 编辑 `api-lines.json`，追加新线路
3. 运行 `scripts/update.py` 生成 `api-lines.js`
4. 提交 PR（参考 [PR 模板](.github/PULL_REQUEST_TEMPLATE.md)）

### 改进播放器
`vip-player.html` 是纯静态单文件，直接修改后提 PR 即可。大改动建议先开 Issue 讨论。
